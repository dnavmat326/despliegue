...
const Hero: React.FC<IHeroAPI> = ({ title, subtitle, btntext, img, sociallinks, videos }) => {
    ...
    const [text, setText] = useState('Boton con useState');
        ...
        w-auto absolute top-0 left-0 right-0 opacity-100 z-10'></div>
                <div className='relative opacity-100 z-20 grid items-center nike-container justify-items-center'>
                    <div className='grid items-center justify-items-center mt-28 md:mt-24'>
                        ...
                        <h1 className='text-6xl lg:text-5xl md:text-4xl sm:text-3xl xsm:text-2xl
                        font-extrabold filter drop-shadow-sm text-slate-200'>{state.text}</h1>
                        <button ref={myNode} className='button-theme bg-slate-200 shadow-slate-200
                        rounded-xl my-5'>{text}</button>
